    ...
    def step(self, action: int):
        ...
        if self.monitor is None:
            if current_behaviour in supervisor_map[action]:
                ...
                # ae predicts too many false positives: episode should not end, but behaviour is normal (because MTD was correct)
                ...
                if self.interpreter:
                    for i in range(9): # real world simulation with 10 samples
                        new_state = np.vstack((new_state, self.sample_behaviour(Behavior.NORMAL)))
