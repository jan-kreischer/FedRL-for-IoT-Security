...
#finalOutput="/tmp/samples_$1_$(date +'%Y-%m-%d-%H-%M')_${timeWindowSeconds}s" # in case of calling with a parameter
finalOutput="online_samples_${num}_$(date +'%Y-%m-%d-%H-%M')_${timeWindowSeconds}s"
tempOutput=temp
...
then
	resourcePlaceholder="${resourcePlaceholder}numEncrypted,"
fi
